...
class EntityRanker:
    ...
    def __init__(self, type_normalizer=None, ontology_distance=None):
        ...
        self.anchor_words = {
            ...
            "puente", "parque", "jardín", "jardin", "barrio", "mercado",
            "centro", "flamenco", "bienal",
        }

        self.bad_words = {
            "cada", "día", "dia", "semana", "historia", "información",
            "informacion", "curiosidades", "evento", "celebra", "descubre",
            "conoce", "planes", "agenda", "turismo", "cultura", "analytics",
            "google", "patrimonio", "cultural", "inmaterial",
            "teléfono", "telefono", "mapa", "salida", "además", "ademas",
        }

        self.bad_verbs = {
            "es", "son", "fue", "eran", "donde", "cuando", "como",
            "aunque", "existe", "existía", "existia", "celebra",
        }
        ...
        self.generic_entities = {
            "sevilla", "flamenco", "cultura", "turismo", "curiosidades",
            "el flamenco", "la sevilla", "patrimonio cultural inmaterial",
            "google analytics", "la plaza",
        }
    ...
    def _name_quality_score(self, name: str) -> float:
        ...
        if "teléfono" in tokens or "telefono" in tokens:
            score -= 4.0

        if "además" in tokens or "ademas" in tokens:
            score -= 3.0

        if "mapa" in tokens or "salida" in tokens:
            score -= 2.5

        if tokens and tokens[0] in {"el", "la", "los", "las"} and not self._has_anchor_word(name):
            score -= 2.5

        if len(tokens) >= 2 and tokens[0] == "sevilla" and tokens[1] == "el":
            score -= 3.5
